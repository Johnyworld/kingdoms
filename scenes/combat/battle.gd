...
var _view: Control
var _squads := {}                 # team → 스쿼드 렌더 노드({node, body, info, hp_fill, color, pos, ...}). 멤버 시각 집약(방식 1). → battle.md
...
func _spawn_structure(building, team: String, vp: Vector2, gate := false) -> void:
	...
	var su := {
		...
		"hp": int(building.gate_hp if gate else building.wall_hp), "building": building, "effects": {},
	}
	_units.append(su)
...
## 한 팀 멤버 유닛을 세로 열 좌표로 만들되(시뮬용 pos), 개별 토큰 대신 팀당 스쿼드 토큰 1개로 렌더(방식 1). → battle.md
func _spawn_team(party, team: String, x: float, vp: Vector2) -> void:
	...
	for i in n:
		...
		_units.append(unit)
	if n > 0:
		_spawn_squad(party, team, Vector2(x, vp.y * 0.5))

## 팀(부대) 스쿼드 토큰 1개를 만든다 — 팀색 몸통 + 정보 라벨(영웅=지휘관명 / 일반=병종 N/총) + 집계 HP 바.
func _spawn_squad(party, team: String, base_pos: Vector2) -> void:
	var node := _make_squad_token(party.token_color)
	_view.add_child(node)
	var is_hero: bool = party.kind == Party.KIND_HERO
	_squads[team] = {
		"node": node, "body": node.get_node("body"), "info": node.get_node("info"),
		"hp_fill": node.get_node("hpfill"), "bar_w": TOKEN_R * 2.4,
		"color": party.token_color, "pos": base_pos, "base_pos": base_pos,
		"name": party.commander_name() if is_hero else party.party_name, "is_hero": is_hero, "team": team,
	}
	node.position = base_pos - Vector2(TOKEN_R, TOKEN_R)
	_sync_squad(team)
...
## 스쿼드 토큰 = 팀색 몸통(살짝 큰 사각) + 위 정보 라벨 + 아래 집계 HP 바(배경 + 초록 채움).
func _make_squad_token(color: Color) -> Control:
	var w := TOKEN_R * 2.4
	var c := Control.new()
	var body := ColorRect.new()
	body.name = "body"
	...
	bg.size = Vector2(w, 6)
	bg.position = Vector2(0, w + 4)
	c.add_child(bg)
	var fill := ColorRect.new()
	fill.name = "hpfill"
	fill.color = Color(0.35, 0.8, 0.35)
	fill.size = Vector2(w, 6)
	fill.position = Vector2(0, w + 4)
	c.add_child(fill)
	return c

## 팀 스쿼드 토큰을 집계 상태로 갱신 — 위치(생존 멤버 중심)·정보 라벨·HP 바·전멸 페이드.
func _sync_squad(team: String) -> void:
	if not _squads.has(team):
		return
	var sq: Dictionary = _squads[team]
	var s := BattleField.squad_summary(_units, team)
	# 생존 멤버 pos의 중심으로 이동(접근·교전 시 스쿼드가 전진). 생존자 없으면 마지막 위치 유지.
	...
	if c > 0:
		sq["pos"] = sum / float(c)
	sq["node"].position = sq["pos"] - Vector2(TOKEN_R, TOKEN_R)
	if sq["is_hero"]:
		sq["info"].text = sq["name"]
	else:
		sq["info"].text = "%s %d/%d" % [sq["name"], s["alive"], s["total"]]
	var frac := 0.0
	if s["max_hp"] > 0:
		frac = clampf(float(s["hp"]) / float(s["max_hp"]), 0.0, 1.0)
	(sq["hp_fill"] as ColorRect).size.x = sq["bar_w"] * frac
	sq["node"].modulate = Color(1, 1, 1, 1.0 if s["alive"] > 0 else DEATH_ALPHA)

## 팀 스쿼드 토큰의 현재 중심 좌표(멤버 공방 연출의 투사체·대미지 숫자 위치). 없으면 원점.
func _squad_center(team: String) -> Vector2:
	return _squads[team]["pos"] if _squads.has(team) else Vector2.ZERO
...
func _process(delta: float) -> void:
	...
	for u in _units:
		...
		if dot > 0:
			u["hp"] -= dot
			_spawn_float(_squad_center(u["team"]), str(dot), HitFeedback.BLEED_COLOR, false)   # 출혈 도트(스쿼드 위치)
			if u["hp"] <= 0:
				_kill(u)   # 도트 사망: 멤버는 스쿼드 카운트로 반영
	...
	# 멤버 시각 집약: 두 팀 스쿼드 토큰을 프레임 끝에 갱신(생존 수·집계 HP·위치).
	_sync_squad("a")
	_sync_squad("b")
...
func _attack(u: Dictionary, t: Dictionary, weapon: String) -> void:
	...
	# --- 타격 연출(combat-feedback.md) — 멤버 공방은 스쿼드 단위. 공격자·표적 모두 멤버(사람). ---
	var atk_c := _squad_center(u["team"])
	var def_c := _squad_center(t["team"])
	if _squads.has(u["team"]):
		_lunge(_squads[u["team"]], def_c)                 # 공격 스쿼드 돌진
	var ht := HitFeedback.hit_text(r)
	_spawn_float(def_c, ht["text"], ht["color"], ht["big"])   # 대미지 숫자/빗나감/막기(표적 스쿼드)
	if r["hit"] and not r["blocked"] and r["damage"] > 0 and _squads.has(t["team"]):
		_hit_react(_squads[t["team"]])                    # 표적 스쿼드 반짝임 + 흔들림
	if r["inflict"] != "":
		_spawn_float(def_c + Vector2(0, -18), HitFeedback.status_text(r["inflict"]), HitFeedback.STATUS_COLOR, false)
	if ItemTypes.weapon_range(weapon) >= 2 or ItemTypes.weapon_throw_range(weapon) > 0:
		_spawn_projectile(atk_c, def_c)   # 원거리·투척 연출(스쿼드→스쿼드)
	if t["hp"] <= 0:
		_kill(t)   # 멤버 사망: 스쿼드 카운트·HP 바로 반영(개별 넉백 없음)
...
func _kill(u: Dictionary, from_pos = null) -> void:
	u["alive"] = false
	if not u.has("node"):
		return   # 멤버(사람)는 개별 토큰이 없다 — 스쿼드 카운트·HP 바로 사망 반영(다음 _sync_squad)
