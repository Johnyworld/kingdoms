...
const FOCUS_PARTY := "focus_party"  # 플레이어 부대 정보 패널 열기(이동 가능하면 선택)
const FOCUS_NPC := "focus_npc"      # NPC 부대 정보 패널 열기(선택·이동 없음)
const ATTACK := "attack"            # 인접한 적 NPC에게 전투 개시
const DESELECT := "deselect"        # 선택 해제 + 정보 패널 닫기

## 우선순위(위에서부터):
##  1. 플레이어 부대 칸 → 부대 우선(FOCUS_PARTY). 단 캠프 위에 서 있고 정보가 이미 열려 있으면
##     (= 같은 칸 두 번째 클릭) CAMP_MENU로 전환.
##  2. NPC 부대 칸 → 선택 중 + 인접(enemy_adjacent)이면 ATTACK, 아니면 FOCUS_NPC(정보). 이동보다 앞 순위.
##  3. 선택 중 + 이동 범위 칸 → MOVE. 건물 칸이어도 이동이 우선(건물은 이동을 막지 않음 → 통행 가능).
##  4. 캠프 칸 → CAMP_MENU (부대가 없거나 범위 밖).
##  5. 그 외 건물 칸(농장 등) → BUILDING_INFO.
##  6. 그 외 → DESELECT.
static func resolve(on_party: bool, on_npc: bool, on_camp: bool, on_building: bool, selected: bool, reachable: bool, info_open: bool, enemy_adjacent := false) -> String:
	if on_party:
		if on_camp and info_open:
			return CAMP_MENU
		return FOCUS_PARTY
	if on_npc:
		if selected and enemy_adjacent:
			return ATTACK
		return FOCUS_NPC
