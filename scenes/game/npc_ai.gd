...
## 지형 규칙(산 불가·숲/습지 반감)과 맵 경계는 HexGrid.movement_ranges가 반영한다.
## 도달 가능한 이동 칸이 없으면(이동력 0, 사방이 산/맵 밖 등) start를 그대로 반환한다(제자리).
static func choose_destination(terrain: TileMapLayer, start: Vector2i, move_range: int, map_w: int, map_h: int, rng: RandomNumberGenerator) -> Vector2i:
	var ranges := HexGrid.movement_ranges(terrain, start, move_range, map_w, map_h)
