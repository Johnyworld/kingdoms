...
func _handle_click(world_pos: Vector2) -> void:
	...
	# 범위 밖 빈 칸 클릭(DESELECT) + 선택·이동 가능 → 그 방향으로 최대 전진(경로 따라 이동력 닿는 데까지). → selection-and-movement.md
	if action == ClickRouter.DESELECT and _selected and party.can_move() and _try_max_advance(party_cell, cell):
		return
	match action:
		ClickRouter.MOVE:
			# 다중 클릭 이동: 선택을 유지한 채 경로를 걸어가고, 완료 시 이동력을 차감하고 범위를 다시 그린다. → selection-and-movement.md
...
## 주인공 부대를 선택하고 이동 범위·공격 가능 적을 표시한다. 정보 패널([소속] 버튼 포함)도 갱신한다.
func _select() -> void:
	_selected = true
...
## 부대 정보 박스 행동 버튼([소속]·[지휘]) 처리. → party-lord.md · squad-stance.md
...
func _stop_player_move() -> void:
	...
	var stop_cell: Vector2i = _move_path[_move_arrived] if _move_arrived < _move_path.size() else _player_move_target
...
func _launch_follow(hero, hero_dest: Vector2i, from_cell: Vector2i) -> void:
	...
	for f in followers:
		...
		if dest != f_cell:
			...
			if path.size() >= 2:
				f.spend_movement(int(dist.get(dest, 0)))   # 걸어간 누적비용만큼 소모(이동력 풀)
...
## 진행 중인 추종 트레일을 즉시 끝낸다(턴 종료 시): 트윈을 죽이고 각 하위부대를 목적지 칸으로 스냅. → squad-stance.md
...
## 추종 트레일을 현재 위치(진행 중인 칸)에서 멈춘다(영웅 ESC 정지 시): 트윈을 죽이고 각 하위부대를 현재 칸으로 스냅. → squad-stance.md
func _stop_follow_trails() -> void:
	...
	for f in _follow_targets:
		if is_instance_valid(f):
			f.position = terrain.map_to_local(_cell_of(f))   # 현재 칸에 스냅(목적지 아님)
	_follow_targets.clear()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if _build_mode:
		...
		return
	# 다중 클릭 이동 중 ESC → 현재 칸에서 정지(간 만큼만 이동력 소모, 선택 유지·재경로). → selection-and-movement.md
	# 공격·점령 접근 이동(_move_path 비어 있음)은 멈추지 않는다(이미 확정된 행동).
