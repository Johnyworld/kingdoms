...
func setup(terrain: TileMapLayer, center_cell: Vector2i, type_id: String, p_under_construction := false, p_buildings_layer: TileMapLayer = null) -> void:
	texture_filter = TEXTURE_FILTER_NEAREST   # _draw 라벨(MapText draw_string) 글리프가 줌 확대에도 선명하게
	_terrain = terrain
...
# 지도 라벨 폰트 크기(월드=16px 헥스 기준). MapText가 슈퍼샘플로 선명하게 그린다.
const _LABEL_FS := 7      # 거점명·세력명
const _BADGE_FS := 6      # 수비/건설 배지

## 수비 인원 표시("수비 N")를 앵커 중앙에 그린다(완성 거점, 중심 점거 방어 부대 있을 때).
func _draw_garrison_badge(anchor: Vector2, count: int) -> void:
	MapText.draw_centered(self, "수비 %d" % count, anchor.x, anchor.y, _BADGE_FS, Color(0.85, 0.85, 0.95))

## 건설 중 표시("건설 중 N")를 앵커 중앙에 그린다.
func _draw_construction_badge(anchor: Vector2) -> void:
	MapText.draw_centered(self, "건설 중 %d" % remaining_turns, anchor.x, anchor.y, _BADGE_FS, Color(1.0, 0.9, 0.4))
...
func _draw_labels(anchor: Vector2) -> void:
	...
	if lines.is_empty():
		return
	var line_h := _LABEL_FS + 2
	# anchor를 맨 아랫줄의 baseline으로 삼아 위로 쌓아 올린다.
	var baseline := anchor.y - (lines.size() - 1) * line_h
	for line in lines:
		MapText.draw_centered(self, line["text"], anchor.x, baseline, _LABEL_FS, line["color"])
		baseline += line_h
