extends Node2D
## 사다리 시각화 — SiegeSystem.ladders를 받아 공격자 셀↔대상 셀 경계에 마커 + 남은 턴을 그린다.
## game.gd가 ladders/terrain을 채우고 사다리 변경 시 재주입 + queue_redraw()한다(밀기/정리는 목록 재할당). → docs/spec/features/wall.md

var terrain: TileMapLayer
var ladders: Array = []   # SiegeSystem.ladders({building, target_cell, from_cell, faction, countdown, hooked}) — 변경 시 game.gd가 재주입
