...
# 사격(RANGED) 타이밍
const SHOT_STAGGER := 0.13  # 라운드 내 화살 발사 간격(초) — 순차 발사
...
func _load_scenario(n: int) -> void:
	...
	match n:
		0:
			_load_melee()
		2:
			_field.call("setup_ranged", "archer", "infantry", 0, 0)  # 필드 비움
			_hud.set_title("시나리오 2 · 슬라이스 2 예정")
			_hint.text = "경궁병 vs 경보병 (근접)은 슬라이스 2 예정.  우측 버튼 = 시나리오,  ESC = 타이틀"
			_state = St.DONE
		_:
			_load_ranged(n)
...
func _process_ranged(delta: float) -> void:
	...
	if _cur_round >= _rounds_shots.size():
		if not _field.call("arrows_active"):   # 화살·사망 애니까지 끝나면 결과
			_finish_ranged()
		return
