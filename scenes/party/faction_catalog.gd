...
## lazy-load 한다(UnitTypes 와 동일 패턴). 병종 표시명·전투 스탯은 UnitTypes(unit_types.csv)가 결정.
...
## 병종 표시 이름(경보병/경궁병). unit_types.csv(UnitTypes)에 위임. 없으면 빈 문자열.
static func troop_name(archetype: String) -> String:
	return UnitTypes.display_name(archetype)
