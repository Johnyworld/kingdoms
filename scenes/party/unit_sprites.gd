...
## 근접 기본 세트(미지원/빈 아키타입 폴백).
const DEFAULT_SET := "soldier"

## 세트별 idle SpriteFrames 정적 캐시(부대들이 공유 — 64부대라 매번 만들지 않는다).
static var _idle_cache := {}

## 아키타입에 해당하는 스프라이트 세트 키. UnitTypes.sprite 단일 출처 위임(맵 토큰·전투 매핑 통일).
## 미지원/빈 값은 경보병(soldier)으로 대체.
static func set_key(archetype: String) -> String:
	var key := UnitTypes.sprite(archetype)
	return key if key != "" else DEFAULT_SET
