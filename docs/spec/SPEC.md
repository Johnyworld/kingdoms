...
- [Squad Command (부대 지휘 — 따라옴·전투 스탠스)](features/squad-stance.md) — 영웅부대 지휘 지속 설정([따라옴|직접명령]·[전투우선|전투회피]), 부대 정보 패널 우측 상단에 인라인 토글 상시 노출. 따라옴이면 영웅 이동마다 하위부대 자동 추종, 전투우선이면 사거리 적 교전
...
| 중세풍 UI 테마(전역) | `assets/ui/medieval_theme.tres` + `assets/ui/darkages/*`(타일시트) + `assets/ui/fonts/Galmuri14.ttf` |
