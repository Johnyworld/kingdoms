...
| 축사(`stable`) | 밀 2 → 고기 1 + **가죽 1** | 가죽 |
| 목장(`ranch`) | 밀 2 → 고기 1 + **천 1** | 천 |
...
- **신규 자원**([Resources](../data/resources.md)): `밀가루`(2)·`은괴`(20)·`금괴`(30)·`가죽`(4)·`천`(4). 캠프 초기자원 0.
- **부산물(다중 산출)**: 레시피 `out`이 자원 여러 개면(축사 `{고기:1, 가죽:1}`, 목장 `{고기:1, 천:1}`) 변환마다 전부 산출한다. 기존 tick 루프(`for res in out: += out[res]*batches`)가 그대로 처리 — 추가 로직 없음. **N개 유지 모드는 출력 첫 자원(주 산출, 여기선 고기)**을 기준으로 판정한다(부산물은 미판정, 함께 나옴).
...
- 가축 다양화(양계장·마구간 등 더 많은 육류/자원)·아이템 제작 건물(대장간 등)은 더 후속. (채석장 1차 전환·부산물(가죽·천)은 완료.)
