...
- [정상] `hit_chance` = `90 − 대상 회피율`(하한 위에서는)
- [경계] `hit_chance` 하한 `MIN_HIT`(10) — 회피가 매우 높아(민첩 200 → 회피 100 → `90−100=−10`) 계산값이 음수여도 `hit_chance == 10`(하한 적용, 불사 방지)
...
### 1회 공방 (`resolve_hit`)
- [경계] 회피율이 매우 높아도(민첩 200 → 회피 100 → 명중 하한 10%) **불사가 아니다** — 여러 시드를 돌리면 `hit=true`(피해>0)와 `hit=false`가 모두 나온다(명중 하한 `MIN_HIT`)
- [정상] 같은 시드 → 같은 결과(결정적)
