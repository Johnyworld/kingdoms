...
판매·구매 패널은 [보급](camp-menu.md) 패널과 함께 표시된다([주둔 부대](garrison.md)가 있고 거점일 때만).
...
*(예전의 「영지 장비」 섹션(영지 `loot_items` 판매)은 [수비대 통합](garrison.md)으로 수비대 노획·영지 귀속이 폐지되면서 함께 제거됐다 — 이제 주둔 부대가 노획물을 직접 보유·판매한다.)*
...
- `[구매]`: 영지 금 ≥ 20 **그리고** 영지 인구 ≥ 1이면 금·인구 차감 + 주둔 부대 `members`에 [`UnitTypes.make_garrison(1)`](../data/units.md)의 소집병 1명 추가. 둘 중 하나라도 모자라면 비활성.
- 주둔 부대가 없으면(구매 패널이 열리지 않음) 병사 구매 불가.
- 구매 후 구매·판매 목록과 좌측 자원 그리드(금·인구)를 갱신하고 거점 수비 배지를 다시 그린다.
...
- [경계] 금 부족 → 그 자원 [구매] 비활성
- [정상] 병사 구매: 주둔 부대 + 영지 금 30·인구 5 → [구매] → 금 10(−20)·인구 4(−1)·주둔 부대 `members` +1
