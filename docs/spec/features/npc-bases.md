...
| 세력 | 모서리 | 캠프 중심 (`NPC_BASES`) | (참고) 부대 오프셋(`NPC_PARTY_OFFSETS`, 거점→중앙) |
| --- | --- | --- | --- |
| 초원 칸국(서) | 북서 NW | `(10, 10)` | `(+5, +5)` |
| 암흑 제국(북) | 북동 NE | `(89, 10)` | `(-5, +5)` |
| 사막 술탄국(동) | 남동 SE | `(89, 89)` | `(-5, -5)` |

- 좌표는 `MARGIN`·`MAP_WIDTH`·`MAP_HEIGHT`로 계산한다(`Vector2i(MAP_WIDTH-1-MARGIN, MARGIN)` 등). 부대는 자기 거점에서 맵 중앙 쪽으로 오프셋만큼 앞에 선다.
