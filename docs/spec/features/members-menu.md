...
- `game.gd._player_faction_members() -> Array` — `MembersMenu.collect_faction_members(PartyManager.units, _player_faction.name)`을 호출한다(모든 플레이어 부대는 `PartyManager.units`에 있음). 오버레이를 **열 때** 이 스냅샷을 넘긴다.
