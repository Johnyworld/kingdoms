...
- **`MOVE`(기본)**: 파랑 이동 범위 + **공격 가능한 적 타일(빨강)** + 중앙 메뉴(이동 전 [사격][휴식][경계] / 이동 후 [사격][대기]). 도달 빈칸 클릭 → 이동, **공격 가능 적 클릭 → 적 팝업**([공격][사격]), 그 외 NPC 클릭 → 정보.
...
> **주둔 중**([Garrison / 주둔](garrison.md))인 부대는 `can_move()`가 거짓이라 이동 범위가 없다(선택해도 파랑 없음) — **[주둔 종료]** 후에만 다시 움직인다. 주둔/주둔 종료는 [부대 행동 메뉴](party-action-menu.md)에서 한다. 자기 세력 거점 중심 타일 위 부대는 그 메뉴에 [주둔]이 뜬다.
...
- **건물은 계속 통행 가능**하다 — 충돌은 **유닛끼리만** 적용된다(건물 통행 규칙은 그대로). **예외: [성벽](wall.md) 있는 적 세력 거점**의 footprint 7칸은 적 부대에게 완전 장애물(진입·통과 불가, 같은 세력은 자유). 세력 상대적이라 부대마다 `blocked_cells`가 다르다.
...
- **이동력은 부대 기준** — `party.movement()`(멤버 이동력의 **최소값**, 가장 느린 멤버 − [과적 페널티](../entities/Party.md#유도-능력치-derived)). 멤버 구성·화물 적재량이 바뀌면 자동 반영(과적이면 범위가 줄고, 2배 용량이면 0 = 이동 불가).
