...
- **`[경계]`**(이동 전만): 각 멤버 **스태미나 10% 회복(반올림)** + **`alert` 부여** 후 턴 종료. alert는 **NPC(적) 턴이 끝난 뒤 해제**(= 내 다음 턴). (구 전투 수학의 ×1.2 버프는 RPG 전투 계층 폐기로 **현재 전투에 미반영** — 플래그만 남음.)
...
- [정상] `apply_alert()` — 스태미나 10%(반올림) 회복 + `alert == true` (구 전투 ×1.2 효과·테스트는 전투 수학 폐기로 삭제)
...
- [휴식] → hp·스태미나 회복 후 턴 종료. [경계] → 스태미나 회복+alert 후 턴 종료, 내 다음 턴에 해제. (alert의 구 전투 ×1.2 효과는 폐기 — 현재 미반영)
