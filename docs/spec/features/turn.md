...
**NPC 이동 — 영웅그룹별 순차 + 시야 내 카메라 포커스**(Slice B): NPC 세력의 이동은 **영웅그룹 단위로 한 그룹씩** 진행하고, 그룹이 플레이어 시야 안이면 카메라를 그리로 옮겨 보여준다(시야 밖이면 즉시 스냅). → [NPC Movement](npc-movement.md#재생-순서세력--영웅그룹--그룹-내-동시).
