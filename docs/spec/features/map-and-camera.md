...
- **이동 방법** (`_process`):
  - 키보드: **WASD** (누르는 동안 연속 팬).
  - 마우스: 화면 가장자리(`EDGE_MARGIN = 24px`)에 커서를 대면 커서가 **방향 화살표**로 바뀌고(`_edge_cursor_shape`: 상하=VSIZE, 좌우=HSIZE, 대각=FDIAG/BDIAG), **좌클릭을 누르는 동안** 그 방향으로 팬한다(`_edge_dir_for` 순수 함수로 방향 판정). 가장자리 좌클릭은 팬 전용 — 유닛 선택·이동으로 넘어가지 않는다.
  - **예외**: 건설 모드·돌격 목표 지정 중에는 엣지 팬·방향 커서를 끈다(커서는 기본 화살표). 돌격 지정 중 가장자리 근처 목표도 클릭할 수 있도록 목표 선택이 팬보다 우선한다. 모달이 열리면 커서를 기본 화살표로 복원한다.
  - 속도: `CAM_SPEED = 450 px/초`.
