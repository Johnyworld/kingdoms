...
| 사망 넉백 | 전투불능(`_kill`, 공격자 위치 전달) | 공격자 반대쪽으로 **껑충 뛰어(위로 아치)** 날아가고 opacity `0.5`로 페이드(랑그릿사1식). 출혈 도트 사망은 공격자가 없어 넉백 없이 페이드만 |

- 수치는 초안: 떠오름 높이 ~40px·0.7초, 반짝임 ~0.12초, 흔들림 ~0.18초, 돌진 ~10px·0.12초, 사망 넉백 ~60px 뒤로·아치 높이 ~30px·0.35초·opacity 0.5.
...
- 전투불능 시 토큰이 공격자 반대쪽으로 껑충 뛰어 날아가며 흐려진다(opacity 0.5).
