...
- `reach`는 부대 사거리(`max(attack_range, 1)`) — 근접(1)은 적에 인접해지는 칸, 원거리(≥2)는 적이 사거리 안에 **처음 들어오는 경로 칸**에서 멈춘다(정확한 최대 사거리 유지가 아니라 "경로를 따라가다 사거리에 든 첫 지점"이다). 지형/시야 차단은 보지 않는다(순수 헥스 거리, 기존 `NpcPlanner.adjacent_enemy`와 동일 한계).
