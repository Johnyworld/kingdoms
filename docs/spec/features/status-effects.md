...
두 경로 모두 유닛 상태에 `effects: {}`를 두고 [Lang Battle](lang-battle.md)에서 적용한다.
...
- **월드맵 턴 이월** — 상태이상은 전투씬 종료와 함께 사라진다. 전투 후 hp가 회복되는 현재 규칙([Lang Battle](lang-battle.md))과 같은 층위. 턴 지속·이월은 `미구현`.
...
- 부여 계기(치명타)·피해 공식은 [Combat](combat.md), 전투 재생은 [Lang Battle](lang-battle.md).
