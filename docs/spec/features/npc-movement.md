...
- **투석 운용**(공격 페이즈): 투석기를 실은 NPC 부대(주로 [수비대](garrison.md))가 **사거리 밴드 4~5 안에 플레이어 표적**(성벽 거점·부대)이 있으면 [투석](siege-engines.md#npc-공성-ai-5c--_npc_attack_phase)한다(battle.gd 통합 전투). 접근하는 플레이어 공성을 방어 포격·대포병 반격. positioning·생산·NPC↔NPC는 후속.
