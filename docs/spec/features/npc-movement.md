...
NPC의 **수비대 병력(소집병) 자동 보충은 개발하지 않는다**([Garrison / 주둔](garrison.md)) — 수비대가 곧 주둔 부대라 별도 병력 보충 로직을 두지 않는다. 초기 각 거점에 주둔 부대 1개(소집병 4명)가 배치될 뿐이고, 후퇴로 돌아온 부대가 자연히 방어를 재건한다(주둔 조작·재편성은 미구현). *(단 **투석기**는 예외 — NPC 수비대가 주기적으로 [투석기를 보충 생산](siege-engines.md#npc-공성-ai-5c5e--_npc_attack_phase_on_turn_ended)한다.)*
