...
**계층 분리**: `NpcAi`(순수 static — 목적지·티어·전력 판정) ← `NpcPlanner`(의사결정 — 표적 조립 `targets_for`·후퇴·포지셔닝·그룹 이동 계획 `plan_group_move`·표적 탐지 `adjacent_enemy`/`adjacent_enemy_camp`/`siege_target_for`) ← `game.gd`(실행·연출 — 애니메이션·전투·카메라·안개). NpcPlanner는 월드 상태를 game.gd의 좁은 조회 인터페이스(`all_parties`/`all_buildings`/`party_on_cell`/`wall_blocked_cells`/`blocked_for`/`breached_by`)로만 읽어, 테스트에서는 스텁으로 대체한다(`test_npc_planner.gd`). 반경 상수(`DEFEND_RADIUS` 5 · `RETREAT_SCAN` 6 · `PRIORITY_SCAN` 8)도 NpcPlanner 소유.
...
- **신중한 교전**(공격 페이즈, `_npc_unit_act`): 인접 **적** 부대(거점 방어 부대 포함 — 방어자도 그냥 부대다)를 치기 전에 `should_engage`로 판단한다. 대상은 **다른 세력**만(`NpcPlanner.adjacent_enemy`가 같은 세력 부대는 제외) 그리고 **적 [성벽](wall.md) 안 부대는 접근 불가라 제외**. 불리하면(false) 그 NPC는 이번 턴 **교전을 건너뛴다**(대기). 무방비 거점 흡수는 전투가 아니므로 그대로 진행.
...
3. **적 성벽 거점 공성**(`_enemy_walled_centers`) — **시즈 부대**면 사거리 밴드(4~5) 포격 위치(`_siege_band_cells`, [공성병기](siege-engines.md#npcnpc-투석-5g)), **비시즈 부대**면 성벽에 붙어 [사다리 설치](wall.md)(`_wall_assault_cells`). 즉 공성병기 없는 부대도 성벽을 능동 공성한다(사다리 우선).
...
- **행동 결정**(유닛별): 자기 거점 중심을 점거한 부대는 겨눈 사다리가 있으면 [사다리 밀기](wall.md#사다리-밀기-방어). 그 밖엔 인접 적 부대(`NpcPlanner.adjacent_enemy`)가 있고 [신중 교전](battle.md)(`should_engage`)이면 전투, 없으면 인접 무방비 적 캠프 흡수, 그것도 없으면 성벽 사다리 설치.
...
- [정상] `adjacent_enemy` — 사거리 내 적 반환(아군 제외), 성벽 안(`wall_blocked_cells`) 수비대 제외
