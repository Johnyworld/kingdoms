...
- **신중한 교전**(공격 페이즈, `_npc_attack_phase`): 인접 **적** 부대(거점 [주둔 부대](garrison.md) 포함 — 이제 수비대도 부대다)를 치기 전에 `should_engage`로 판단한다. 대상은 **다른 세력**만(`_adjacent_enemy`가 같은 세력 부대·자기 수비대는 제외 — 주둔 부대 도입 후 필요) 그리고 **적 [성벽](wall.md) 안 부대는 접근 불가라 제외**. 불리하면(false) 그 NPC는 이번 턴 **교전을 건너뛴다**(대기). 무방비 거점 흡수는 전투가 아니므로 그대로 진행.
...
- **성벽 공성**(공격 페이즈): 칠 적 부대·흡수할 무방비 거점이 없고 **인접한 성벽 적 거점에 빈 면**이 있으면 [사다리를 설치](wall.md#설치-npc-공성-ai--_npc_attack_phase)한다(전력 판단 없이). 준비된 사다리 통로로는 위 교전·흡수 분기가 자연히 성벽 안으로 이어진다.
...
NPC의 **수비대 병력 자동 보충은 개발하지 않는다**([Garrison / 주둔](garrison.md)) — 수비대가 곧 주둔 부대라 별도 병력 보충 로직을 두지 않는다. 초기 각 거점에 주둔 부대 1개(경보병 10명 — [시작 편제](parties.md))가 배치될 뿐이고, 후퇴로 돌아온 부대가 자연히 방어를 재건한다(주둔 조작·재편성은 미구현). *(단 **투석기**는 예외 — NPC 주둔 수비대가 주기적으로 [투석기를 보충 생산](siege-engines.md#npc-공성-ai-5c5e--_npc_attack_phase_on_turn_ended)한다.)*
...
- **재생 순서**(세력 → 영웅그룹 → 그룹 내 동시):
  - **세력 간 순차** — 한 세력의 모든 그룹 이동이 끝나야 다음 세력이 시작한다.
  - **영웅그룹별 순차** — 한 세력 안에서 **영웅부대 + 그 소속 하위부대(`lord==영웅`)를 한 그룹**으로 묶어(`NpcAi.hero_groups`) **한 그룹씩** 이동시킨다(A그룹 이동 완료 → B그룹). 영웅 없는 독립 이동 부대는 단독 그룹.
