...
- **목적지 선택**(`NpcAi.choose_destination`):
  1. `HexGrid.movement_ranges(terrain, start, move_range, ...)`로 이동 가능한 목적지 집합(`move`)과 거리 맵(`dist`)을 구한다. 지형 규칙(산 진입 불가·숲 `ceil`·습지 `floor` 반감)과 맵 경계는 이 헬퍼가 이미 반영한다([Selection & Movement](selection-and-movement.md)).
...
- **이동 헬퍼**(`game.gd` `_animate_path`): 부대를 경로의 칸을 차례로 지나도록 Tween으로 이동시키고, 각 칸 도착 시 콜백(NPC는 자기 시야 표시 토글)을 실행한다.
- **속도**: 칸당 `MOVE_STEP_TIME`(0.12초). 이동력 4칸이면 약 0.5초.
- **재생 순서**:
...
- **유닛 충돌·점유** — NPC끼리·플레이어·건물 위로 **겹칠 수 있다**. `movement_ranges`가 유닛을 장애물로 취급하지 않기 때문이다(건물 통행과 같은 이유). *(다음 단계)*
