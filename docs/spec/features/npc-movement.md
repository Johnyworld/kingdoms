...
- **신중한 교전**(공격 페이즈, `_npc_unit_act`): 인접 **적** 부대(거점 방어 부대 포함 — 방어자도 그냥 부대다)를 치기 전에 `should_engage`로 판단한다. 대상은 **다른 세력**만(`_adjacent_enemy`가 같은 세력 부대는 제외) 그리고 **적 [성벽](wall.md) 안 부대는 접근 불가라 제외**. 불리하면(false) 그 NPC는 이번 턴 **교전을 건너뛴다**(대기). 무방비 거점 흡수는 전투가 아니므로 그대로 진행.
- **사다리 밀기(성벽 방어)**: 자기 거점 중심을 점거한 NPC 부대는 그 거점을 겨눈 사다리가 있으면 [사다리를 민다](wall.md#사다리-밀기-방어)(공격 페이즈, `_can_push_ladder`). *(예전 `stationed`(주둔) 상태·주둔 중 사격·이동 제외는 [주둔 제거](camp-capture.md)와 함께 삭제 — 방어자는 이제 일반 부대로 이동·전투하고, 거점 방어는 후퇴로 돌아온 부대가 중심을 점거해 창발한다.)*
...
NPC의 **수비대 병력 자동 보충은 개발하지 않는다** — 수비대가 곧 일반 부대라 별도 병력 보충 로직을 두지 않는다. 초기 각 거점에 부대 1개(경보병 10명 — [시작 편제](parties.md))가 중심을 점거해 방어할 뿐이고, 후퇴로 돌아온 부대가 자연히 방어를 재건한다([거점 방어](camp-capture.md#거점-방어-창발--중심-점거)). *(예전 NPC 투석기 주기 보충 생산은 [주둔·생산 제거](camp-capture.md)와 함께 삭제 — [Siege Engines](siege-engines.md).)*
...
- **행동 결정**(유닛별): 자기 거점 중심을 점거한 부대는 겨눈 사다리가 있으면 [사다리 밀기](wall.md#사다리-밀기-방어). 그 밖엔 인접 적 부대(`_adjacent_enemy`)가 있고 [신중 교전](battle.md)(`should_engage`)이면 전투, 없으면 인접 무방비 적 캠프 흡수, 그것도 없으면 성벽 사다리 설치.
