...
- **신중한 교전**(공격 페이즈, `_npc_unit_act`): 인접 **적** 부대(거점 [주둔 부대](garrison.md) 포함 — 이제 수비대도 부대다)를 치기 전에 `should_engage`로 판단한다. 대상은 **다른 세력**만(`_adjacent_enemy`가 같은 세력 부대·자기 수비대는 제외 — 주둔 부대 도입 후 필요) 그리고 **적 [성벽](wall.md) 안 부대는 접근 불가라 제외**. 불리하면(false) 그 NPC는 이번 턴 **교전을 건너뛴다**(대기). 무방비 거점 흡수는 전투가 아니므로 그대로 진행.
- **주둔 부대**: `stationed`인 NPC 부대는 이동 페이즈에서 **건너뛴다**(거점에서 대기). 공격 페이즈에서는 근접 개시는 안 하되, **원거리 무기 + 사거리 안 적이 있으면 [주둔 중 사격](garrison.md#주둔-중-사격-party_action_menu--gamegd--_npc_unit_act)한다**(주둔 유지, 턴당 1회). 공격받으면 일반 부대로 방어한다. → [Garrison / 주둔](garrison.md).
- **성벽 공성**(공격 페이즈): 칠 적 부대·흡수할 무방비 거점이 없고 **인접한 성벽 적 거점에 빈 면**이 있으면 [사다리를 설치](wall.md#설치-npc-공성-ai--_npc_unit_act)한다(전력 판단 없이). 준비된 사다리 통로로는 위 교전·흡수 분기가 자연히 성벽 안으로 이어진다.
- **투석 운용**(공격 페이즈): 투석기를 실은 NPC 부대가 **사거리 밴드 4~5 안 적 표적**(적 세력 성벽 거점·부대)이 있으면 [투석](siege-engines.md#npcnpc-투석-5g)한다(플레이어 표적은 battle.gd 오버레이, NPC 표적은 헤드리스). 로빙 NPC는 성벽 밴드에 자리잡고(positioning, 5f) 능동 포격, NPC↔NPC 성벽·부대 투석까지 지원(5g).
...
3. **적 성벽 거점 공성**(`_enemy_walled_centers`) — **시즈 부대**면 사거리 밴드(4~5) 포격 위치(`_siege_band_cells`, [공성병기](siege-engines.md#로빙-positioning-공격형-공성-5f--_setup_parties_npc_targets)), **비시즈 부대**면 성벽에 붙어 [사다리 설치](wall.md)(`_wall_assault_cells`). 즉 공성병기 없는 부대도 성벽을 능동 공성한다(사다리 우선).
4. **나머지**(전체 적 셀, 기존 `enemy_cells`) — 가장 가까운 것으로 접근(폴백).
...
NPC의 **수비대 병력 자동 보충은 개발하지 않는다**([Garrison / 주둔](garrison.md)) — 수비대가 곧 주둔 부대라 별도 병력 보충 로직을 두지 않는다. 초기 각 거점에 주둔 부대 1개(경보병 10명 — [시작 편제](parties.md))가 배치될 뿐이고, 후퇴로 돌아온 부대가 자연히 방어를 재건한다(주둔 조작·재편성은 미구현). *(단 **투석기**는 예외 — NPC 주둔 수비대가 주기적으로 [투석기를 보충 생산](siege-engines.md#npc-공성-ai-5c5e--_npc_unit_act_on_turn_ended)한다.)*
...
- **재생 순서**(세력 → 영웅그룹 → 그룹마다 이동→공격):
  ...
  - **영웅그룹별 순차, 그룹마다 [이동 → 공격]** — 한 세력 안에서 **영웅부대 + 그 소속 하위부대(`lord==영웅`)를 한 그룹**으로 묶어(`NpcAi.hero_groups`) **한 그룹씩** 처리한다. **그룹이 먼저 이동을 마친 뒤, 그 그룹이 공격**하고(아래 [NPC 공격](#npc-공격-그룹-이동-직후)), 다음 그룹으로 넘어간다. 영웅 없는 독립 부대는 단독 그룹. 이동 계획(목적지·경로)은 **그 그룹 차례에 즉석 수립**하되, **점유 회피(`_blocked_for`)만 실시간**이다 — 표적 목록(플레이어 부대·캠프)은 턴 시작 스냅샷이라, 앞 그룹이 죽인 부대·뺏은 캠프는 표적 선택에 즉시 반영되지 않을 수 있다(회피는 반영).
  - **그룹 내 동시(스태거)** — 한 그룹의 부대들은 함께 움직이되 `NPC_PARTY_STAGGER`(0.2초)씩 시작을 늦춘다. 주둔 부대는 이동하지 않는다.
...
- **주둔 영웅**: 영웅이 거점에 주둔(`stationed`) 중이면 제자리를 목적지로 삼아, 하위부대가 영웅 곁(거점)에 대형/수비하도록 한다(흩어지지 않음).
...
- **행동 결정**(유닛별): 주둔 부대는 투석기 방어 포격 → 사다리 밀기 → 사거리 안 적 제자리 사격. 그 외 부대는 인접 적 부대(`_adjacent_enemy`)가 있고 [신중 교전](battle.md)(`should_engage`)이면 전투, 없으면 인접 무방비 적 캠프 흡수, 그것도 없으면 성벽 사다리 설치. (Slice A~B 이전과 같은 판정 — 순서만 그룹 단위로 정리.)
...
- **전투 결산**:
  - **플레이어 부대가 대상** → [전투 오버레이](battle.md)로 관전(`_run_battle`).
...
- [정상] `Party.melee_power()`/`ranged_power()` — 검 든 멤버 부대는 melee_power = 검 공격력 합·ranged_power 0; 활 든 멤버는 ranged_power = 활 공격력 합
- [정상] 검+활 든 멤버는 melee_power·ranged_power 둘 다 기여(각 최선 무기)
- [경계] 멤버 없으면 둘 다 0
