...
- **신중한 교전**(공격 페이즈, `_npc_attack_phase`): 인접 적 **부대**나 **수비대**를 치기 전에 `should_engage`로 판단한다. 불리하면(false) 그 NPC는 이번 턴 **교전을 건너뛴다**(대기). 무방비 캠프 흡수는 전투가 아니므로 그대로 진행.
...
## 수비대 보충 (`_npc_attack_phase` → `_reinforce_garrison`)

NPC는 공격 페이즈에서 칠 적(부대·적 캠프)이 없을 때, **자기 캠프에 인접**하고 그 캠프 [수비대](garrison.md)가 부족하면(`< NPC_GARRISON_TARGET` = 4) 부대원을 수비대로 채운다.

- 부족분만큼(목표 4까지) 부대 앞에서부터 수비대로 옮긴다(`Party.remove_member` → `garrison.append`). 보충은 그 부대의 이번 턴 행동을 끝낸다(`mark_attacked`).
- 부대가 **비면**(전원 보충) 그 부대는 캠프 수비로 흡수돼 맵에서 사라진다(`_npc_parties`에서 제거·free).
- 후퇴([약하면 후퇴](#약하면-후퇴))로 돌아온 부대가 자연히 방어를 재건한다 → 플레이어가 수비대를 깎아도 다시 채워져 정복이 어려워진다.
- 시작 시 수비대는 4로 가득 차 있어(초기엔 보충 안 함), 플레이어가 공격해 깎은 뒤 NPC가 복귀해야 발동한다.
