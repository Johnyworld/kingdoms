...
- **플레이어 부대**(`UnitTypes.PLAYER_ID` = `azel`, 푸른 왕국)
  ...
  - 플레이어 거점(남서 모서리 캠프) 바로 아래 칸에 배치한다.
...
각 부대는 **자기 세력 거점(모서리) 기준 오프셋**에 배치한다. 4왕국이 네 모서리에 흩어져 있어 시작 시엔 서로 멀다. 거점 좌표는 [NPC Bases](npc-bases.md)(`PLAYER_BASE`·`NPC_BASES`).

| 부대 | 거점(모서리) | 오프셋 (칸) |
| --- | --- | --- |
| 플레이어(아젤) | 남서 `(10,89)` | `(0, +3)` (캠프 아래) |
| 초원 칸국(바트르 칸) | 북서 `(10,10)` | `(+5, +5)` |
| 암흑 제국(발타자르) | 북동 `(89,10)` | `(-5, +5)` |
| 사막 술탄국(카심) | 남동 `(89,89)` | `(-5, -5)` |
