...
- `nearest_enemies(unit, units, n) -> Array` — `nearest_enemy`의 다중판 — 다른 팀 살아있는 유닛을 **거리순 최대 `n`명** 반환(가까운 순, 없으면 빈 배열). [투석기 광역 공격](siege-engines.md#유닛-투석--battlegd-통합-전투)(한 발에 최대 5명)이 쓴다. 공성 전투원은 표적에서 제외.
...
- [정상] `nearest_enemies(unit, units, 5)` — 다른 팀 살아있는 적을 거리순 최대 5명(가까운 순)
- [경계] `nearest_enemies` — 적이 n보다 적으면 있는 만큼, 적 없으면 빈 배열, 죽은 적·같은 팀 제외
