...
- `_apply_survivors(party, survivors)` — 각 부대의 `members`를 **생존자로 교체**한다. 지휘관이 사망했으면 생존 멤버 중 첫 명으로 재지정한다.
  ...
  - **플레이어 부대가 전멸**하면 NPC와 마찬가지로 맵에서 제거한다(`_units`에서 제외, `queue_free`). 그 부대가 **활성 부대(`party`)** 였으면 선택을 해제하고 `party`를 **남은 살아있는 플레이어 부대**로 재할당한다(없으면 `null`, [부대 정보](party-info.md) 닫기). **부대가 0이 돼도 패배가 아니다** — 캠프 수비대에서 [새 부대를 편성](party-composition.md#부대-생성--캠프-수비대에서-편성-_raise_party)해 복귀할 수 있고, 패배는 [세력 소멸](victory.md)(거점 0)로만 난다.
