...
- `game.gd`가 개시 시 공격자·대상의 **헥스 거리**를 구해 전달한다(근접 [공격] = 1, [사격] = 부대↔표적 헥스 거리). *(투석기·구조물 등 사거리 5 병기는 [공성병기 5d-2/3](siege-engines.md#공성병기-로드맵)에서 이 거리 모델 위에 얹는다.)*
...
- `nearest_enemy(unit, units) -> Dictionary` — `unit`과 **다른 팀**의 **살아있는** 유닛 중 `pos` 거리가 가장 가까운 것. 없으면 빈 Dictionary(`{}`). 오버레이가 매 프레임 대상 재탐색(상대 사망 시 새 상대)에 쓴다. *(공성 전투원(`siege`)은 표적에서 제외 — [투석기 피격은 5d-3](siege-engines.md#공성병기-로드맵).)*
- `bombard_targets(unit, units, n) -> Array` — [투석기 광역 공격](siege-engines.md#투석기-피격파괴-방어-카운터플레이) 표적(한 발 최대 `n`명). 다른 팀 살아있는 유닛 중 **적 공성 전투원(`siege`)을 우선**, 그다음 일반 유닛, 각 그룹 내 거리순으로 최대 n명(없으면 빈 배열). `nearest_enemy`와 달리 **공성 전투원을 표적에 포함**한다(대포병 결투 — 투석기끼리 서로 노림).
...
- [정상] `bombard_targets(unit, units, 5)` — 적 공성 전투원 우선 → 일반 유닛, 각 거리순 최대 5명
- [정상] 뒤에 있는 적 투석기라도 일반 유닛보다 먼저 뽑힌다(대포병 우선)
