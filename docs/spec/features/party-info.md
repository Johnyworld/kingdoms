...
- 세로(VBox)로 쌓는다:
  ...
  - **요약** — `"이동력 N · 시야 M · 사거리 <근접|N>"` (`party.movement()`·`party.vision()`·`ItemTypes.range_label(party.attack_range())` — 근접 무기만이면 "근접"). `party.movement()`는 [과적 감소](../entities/Party.md#유도-능력치-derived) 반영된 값이며, **과적이면 뒤에 `· 과적 −K`** 를 붙인다(`party.overload_penalty() > 0`일 때, K=페널티).
  - `HSeparator`.
...
- `open(party) -> void` — 부대 정보를 채우고 패널을 보인다.
  ...
  - 요약 = `"이동력 %d · 시야 %d · 사거리 %s"` (`party.movement()`·`party.vision()`·`ItemTypes.range_label(party.attack_range())`) + 과적이면 `" · 과적 −%d"`(`overload_penalty()`).
...
- [정상] 과적 부대(화물 용량 초과) `open` → 요약에 `"과적 −"` 포함, 이동력 숫자는 감소분 반영; 과적 아니면 `"과적"` 없음
