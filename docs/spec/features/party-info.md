...
- 세로(VBox)로 쌓는다:
  ...
  - **행동 버튼 줄**(HBox) — `open`에 넘긴 `actions`(각 `{id, label}`) 버튼을 가로로 놓는다. 비어 있으면 이 줄을 숨긴다. 중앙 메뉴가 없어져 부대 조작 버튼이 이 박스로 들어왔다: **일반부대 선택** 시 소속 관리 가능하면 `[소속]`([Party Lord](party-lord.md)), **이동 목표가 남은 부대**면 `[계속 이동]`([Selection & Movement](selection-and-movement.md#이동-목표-기억계속-이동-move_goal) — 목표로 최대 전진), **아직 할 게 남은 부대**(명령 남음 = E가 아님)면 `[대기]`([Turn](turn.md#턴-종료-버튼-turn_hudgd) — 남은 이동력·공격을 포기하고 강제 E). 버튼 클릭 → `action_selected(id)` 방출(`game.gd`가 처리). NPC·조작 대상 아닌 부대는 `actions`가 비어 버튼 줄이 없다.
  - **[대기]** 처리(`game.gd._on_party_info_action("wait")`): `party.wait()`로 강제 소진 → 선택 해제·정보 패널 닫기·`_update_fog`(E 배지·"명령 남음" 카운터 갱신).
  - **지휘 토글**(HSeparator + 2줄) — **영웅부대**를 선택하고 명령 가능한 하위부대가 있을 때만 행동 버튼 아래로 상시 노출한다(`open`의 `show_command`). 추종 `[따라옴|직접명령]`·전투 `[전투우선|전투회피]`, 현재 값 쪽 버튼은 밝게 강조(불투명+금색 글자, 선택 표시)하고 반대쪽은 흐리게 둔다. 누르면 `command_changed(field, value)` 방출 → `game.gd`가 `command_follow`/`command_engage`를 세팅·재렌더. 구 [지휘] 모달을 흡수한 것. → [Squad Command](squad-stance.md)
