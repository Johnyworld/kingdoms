...
- 세로(VBox)로 쌓는다:
  ...
  - **요약** — `"이동력 N · 시야 M · 사거리 <근접|N>"` (`party.movement()`·`party.vision()`·`ItemTypes.range_label(party.attack_range())` — 근접 무기만이면 "근접"). (화물 제거로 과적 표시는 폐지.)
  - `HSeparator`.
  - **멤버 리스트**(VBox) — 멤버 한 명당 라벨. 이름·이동·시야 + **무기 줄**:
    - `"<이름>   HP <hit_points>/<max_hp()>   이동 <movement> / 시야 <vision>"` (HP는 [Human](../entities/Human.md)의 현재 생명점 / 계산된 최대 생명점 — 전투 후 지속됨)
    - `"  <주무기이름>[ (+<보조무기들>)]"` — 무기이름은 [ItemTypes](../data/items.md), 없으면 `"맨손"`. **무기를 여럿 들면** 주무기(첫 원소) 뒤에 보조무기 이름을 `" (+활)"`처럼 덧붙인다. (구 공격/방어/회피/막기 표시는 RPG 전투 수학 폐기로 제거 — 전투 판정은 lang 클래스.)
    - `"  방어구: <조각 이름들, 콤마 구분>"` — 착용 방어구 조각 이름(`ItemTypes.armor_name`). **방어구가 없으면(맨몸) 이 줄은 표시하지 않는다.**
...
- `open(party) -> void` — 부대 정보를 채우고 패널을 보인다.
  ...
  - 멤버 리스트를 **비우고** 다시 채운다(재오픈 시 이전 멤버가 남지 않도록). 각 멤버 = 이름·HP(현재/최대)·이동·시야 줄 + 무기 줄(무기 이름) + 방어구 줄(착용 시).
- `close() -> void` — 숨긴다.
...
- [정상] 무기 장착 멤버 라벨에 무기 이름이 포함됨(공격/방어/회피/막기 표시는 전투 수학 폐기로 제거)
