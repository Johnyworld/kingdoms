...
- **플레이어(수동)**: 주둔 부대를 선택했을 때 그 부대가 원거리 무기(사거리 ≥ 2)를 갖고 **현재 위치 사거리 안에 사격 가능 적**이 있으면, 행동 메뉴에 **[사격]**을 [주둔 종료]·[장비]와 함께 넣는다. [사격] → 그 적을 **제자리 원거리 전투**([Battle](battle.md), SHOOT 모드)로 친다.
  - **주둔은 유지**된다(주둔 종료 아님). 발사는 그 턴 행동을 소비해 **턴당 1회**(`attacked_this_turn`) — 다음 턴 리셋 후 다시 사격 가능(주둔 지속).
  - 사격거리 안 적이 없거나 원거리 무기가 없으면 [사격]은 뜨지 않는다.
- **NPC(자동)**: NPC 공격 페이즈에서 주둔 NPC 수비대가 원거리 무기 + 사거리 안 적을 두면 **자동 사격**(주둔 유지, 턴당 1회). 근접 이동·개시는 여전히 안 함(대기). → [NPC Movement](npc-movement.md).
...
- **[성벽](wall.md) 있는 적 거점**은 적 부대가 footprint에 진입할 수 없어 중심 수비대에 접근 못 함 → **공격·점령 모두 불가**(footprint 안 부대는 표적에서 제외). 사다리·공성병기(후속 슬라이스)로만 돌파.
...
`game.gd`의 초기 주둔 부대 배치·방어/점령 게이트·주둔 종료 후 이동·NPC 주둔 제외·주둔 사격(플레이어 수동 발사·NPC 자동 발사)은 씬 트리·터레인 의존이라 실제 실행으로 확인한다. *(game.gd 통합 테스트는 기존 관례상 두지 않음)*
