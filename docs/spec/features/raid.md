...
- 패널 chrome(딤 배경·제목 바·X·ESC·지도 입력 차단)은 **공용 [Modal](modal.md)에 위임** — 제목 바 = `"약탈 — <패자 부대명>"`(부대명 없으면 `"적 부대"`). 콘텐츠는 **좌우 2열** —
  ...
  - 하단 **[모두 가져오기]**(남은 장비 전부). (별도 [닫기] 버튼 없음 — 닫기는 Modal chrome의 X·배경 좌클릭·ESC.)
- 왼쪽에서 **[가져오기]** 하면 그 행이 사라지고 오른쪽 내 인벤토리에 반영된다. 노획 대상 장비가 모두 비거나 닫으면(X·배경·ESC) 패널을 닫는다. 어느 경로로 닫혀도 `closed` 시그널로 수렴한다(`game.gd` await 재개).
...
### 내 인벤토리 묶음 표시 · Modal 개폐 — `test/unit/test_loot_menu.gd`

- [정상] `_grouped_lines(["sword","sword","bow"]) == ["검 ×2", "단궁 ×1"]`(이름별로 묶고 첫 등장 순서 유지)
- [경계] `_grouped_lines([]) == []`
- [정상] `open` → 내부 Modal 열림(제목 `"약탈 — <부대명>"`), 닫으면 `closed` 방출
- [정상] [모두 가져오기] → 승자 `loot_items`로 전량 이동 + 자동 닫힘(`closed` 방출)
