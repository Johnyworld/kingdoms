...
chrome(딤 배경·제목 바·X·ESC·`ModalStack` 등록 = 지도 입력 차단)은 **공용 [Modal](modal.md)에 위임**하고, 콘텐츠(두 패널)만 `set_content`로 주입한다(공용 Modal 컴포지션 패턴).
...
- **event-driven 자동 갱신**: 열 때 영지의 [`changed` 시그널](../entities/Territory.md)을 구독한다(다른 영지로 재오픈 시 구독 교체). 열려 있는 동안 자원·건물·세력이 바뀌면 **다음 idle 프레임에 한 번만** `_refresh`한다(코얼레싱 — 같은 프레임에서 시그널 뒤에 오는 `upgrade_to` 등 건물 변경까지 최종 상태로 반영). 업그레이드 후 game.gd의 수동 재-open은 없다. `_refresh`는 정보 화면(리스트 숨김·건축 버튼 표시)으로 초기화한다.
...
- **건축 버튼** (`_on_build_pressed`) — 우측 패널을 **건설 리스트**로 전환한다(건축 버튼은 숨기고 리스트를 보임).
  - 리스트 = [건물 카탈로그](../data/buildings.md)의 **건축 가능 종류**(`BuildingTypes.BUILDABLE_IDS` — 농장·벌목소·철광·금광·집). 거점(캠프·마을회관·성)은 제외(캠프=새 영지, 마을회관·성=업그레이드).
...
- **(삭제됨) 보급(화물)·판매·구매·병사 패널** — **화물운반**(영지↔부대 자원 적재/하역)은 [화물 제거](../entities/Party.md)와 함께 삭제됐다(부대가 자원을 나르지 않음). 상거래(판매·구매·병사)도 이미 제거됨. 캠프 메뉴는 이제 자원 그리드 + 영지 정보 + 건축/업그레이드/캠프건설/철거 버튼만 띄운다.
...
- [정상] 자원 충분 + **캠프 거점**(tier 0) 영지로 건축 → **농장·벌목소·철광·금광**(선행 camp)이 **활성**; **집**(선행 town_hall)은 **비활성**이고 라벨에 `"(선행: 마을회관 필요)"` 포함
- [정상] 건축 리스트 항목 수 = 5(농장·벌목소·철광·금광·집, 거점 미포함), 농장 텍스트에 라벨·비용 포함(인원 표기 없음)
- [정상] 거점을 **마을회관**(tier 1)으로 하면 집이 **활성**으로 전환
