...
**캠프** 헥스를 클릭하면 열리는 오버레이. 그 건물이 속한 **영지** 정보(자원·이름·세력)를 보여준다.
(캠프가 아닌 건물은 대신 [건물 정보 패널](building-info.md)이 열린다.)
...
- [정상] `인구` 행 값이 `"현재 / 상한"` 형식(예: 캠프만 있는 영지 → `"10 / 10"`)
- [정상] 자원 충분 + **완성 캠프**가 있는 영지로 열고 건축 → 리스트에 **마을회관·채석장**(선행 camp)이 **활성**
- [정상] 항목 텍스트에 라벨과 비용(자원명·수량)이 포함된다
- [경계] 마을회관 없는 영지(캠프만) → **농장·집·벌목소**(선행 town_hall)는 **비활성**이고 라벨에 `"(선행: 마을회관 필요)"` 포함
- [정상] 완성 **마을회관**을 영지에 추가하고 재오픈·건축 → 농장·집·벌목소가 **활성**으로 전환
- [정상] 채석장 항목 텍스트에 `"인원 1"` 포함(required_pop > 0)
- [경계] 인구가 `required_pop` 미만인 영지 → 선행·자재 충족이어도 해당 항목 **비활성**
- [경계] 자원 부족한 영지로 열고 건축 → (선행 충족이어도) 해당 항목 **비활성**(`disabled`)
- [경계] 영지 없는 건물로 열고 건축 → 모든 항목 **비활성**
