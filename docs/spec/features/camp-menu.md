...
- **구매 패널** (`_buy_panel`, `party != null` + **거점**일 때 — 판매 패널과 같은 조건) — 금으로 장비를 사서 부대 `loot_items`에 넣는다([Trade](trade.md)). `ItemTypes` 전 카탈로그를 무기/방어구/방패 섹션으로 나열, 각 행 `"<이름> <구매가>금"` + `[구매]`. **구매가 = `item_value × BUY_MARKUP`(2)**. `[구매]`는 영지 금 ≥ 구매가면 금 차감 + `loot_items` 추가, 금 부족이면 비활성. 구매할 때마다 구매·판매 목록과 좌측 자원 그리드(금)를 다시 그린다.
