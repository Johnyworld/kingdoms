...
- **화물·노획 장비 분배**: 멤버 목록 아래 두 섹션 —
  - **화물**: 자원별 행 `자원 · 원N [→][←] 새M`. `[→]`/`[←]`가 `CARGO_STEP`(5)씩 원↔새로 옮긴다(`Party.transfer_cargo_to`). **받는 부대 [화물 용량](../entities/Party.md#화물-cargo--캐러반)(50) 초과 허용**(병합·약탈과 동일) — 그 방향 **보유가 0일 때만** 비활성. `인구`는 제외(노동력). *(적재 초과 시 이동력 감소는 `미구현`(예정).)*
