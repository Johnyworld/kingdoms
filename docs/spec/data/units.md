...
`start_corner`는 거점 배치의 **데이터 부분**이다 — 실제 좌표는 `game.gd`의 `corner_cell(corner, map_w, map_h, margin)`가 맵 크기·`MARGIN`으로 계산한다([NPC Bases](../features/npc-bases.md) · [Map & Camera](../features/map-and-camera.md)).
...
| `troop_name(archetype) -> String` | String | 병종 표시 이름(경보병/경궁병) — `GameUnits.display_name`에 위임. 없으면 빈 문자열 |
...
`test/unit/test_unit_types.gd` · `test/unit/test_game_units.gd` · `test/unit/test_game_placement.gd`.
