...
> **자원 4종 축소**에 맞춰 생산 지형은 **초원(농장·식량)·숲(벌목소·목재)·철맥(철광·철)·금맥(금광·금)** 넷만 둔다. 초원·사막·습지·산은 이동/지형 다양성용, 철맥·금맥은 [1차 생산 건물](../features/production.md)의 `buildable_terrains` 대상.
> **제거됨**: `돌(5)`·`동물(6)`·`물가(7)`·`은맥(10)` — 대응 건물(채석장·사냥터·낚시터·은광)이 삭제되면서 함께 제거. `Terrain` 상수·`CATALOG`·맵 생성에서 빠졌다. **남은 id는 재번호하지 않는다**(철맥 8·금맥 9 유지 — 타일셋 참조 안정). 그래서 id에 5~7·10 공백이 있다. (타일셋 소스·플레이스홀더 SVG는 참조가 없어 **미사용**으로 남는다 — 에셋 정리는 후속.)
...
- **맵 생성기**: 현재는 시작 지점 근처에 방향별 소규모 덩어리(서=숲·동=습지·북=사막·남=산 + 생산 지형(철맥·금맥) 흩뿌림)만 고정 배치([map-and-camera.md](../features/map-and-camera.md)). 맵 전역 절차적 생성·바이옴 규칙은 미구현.
