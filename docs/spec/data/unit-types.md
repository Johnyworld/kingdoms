...
병종(unit type) 아키타입을 **데이터로 정의**하는 단일 출처. 전투 스탯(at/df·이동력 mv·지휘범위 cmd_range·지휘보정 cmd_at/cmd_df)·병종 kind·HP·시야·원거리 여부·표시명·**전투씬 스프라이트 세트(sprite)**가 모두 이 카탈로그 한 곳에서 나온다. 세력·영웅은 [FactionCatalog](factions.md), 맵에 놓이는 초기 유닛은 [UnitSpawns](unit-spawns.md)가 담당한다.
...
- `sprite` — 전투씬 캐릭터 애셋 세트 키(`assets/units/<sprite>_*.png`). **스프라이트는 side(공/방)가 아니라 이 필드로 결정**된다([Lang Battle](../features/lang-battle.md) "병사 스프라이트"). 세력 정체성이 공격·방어와 무관하게 유지되고, 세력별 외형은 그 세력이 쓰는 아키타입으로 갈린다(예: 암흑세력은 오크 병종을 사용 — [Unit Spawns](unit-spawns.md)). `combat_stats`(전투 수학)에는 포함하지 않고 렌더 경로(`LangBridge._cfg_side`)만 소비한다.

| 병종 | id | kind | at/df | sprite | 성격 |
| --- | --- | --- | --- | --- | --- |
| (영웅) | `hero` | `hero` | 27/24 | `sword` | 지휘관(표시명 없음 — `hero_name` 사용), cmd_range 4 |
| 경보병 | `light_infantry` | `infantry` | 23/21 | `soldier` | 근접, cmd_range 3 |
| 경궁병 | `light_archer` | `archer` | 23/21 | `archer_a` | 원거리(부대 [공격거리](../entities/Party.md) 3), 경보병과 동일 base |
| (오크 영웅) | `dark_hero` | `hero` | 27/24 | `eliteorc` | 암흑세력 지휘관 — `hero`와 스탯 동일, 외형만 엘리트 오크 |
| 오크 | `orc_infantry` | `infantry` | 23/21 | `orc` | 암흑세력 근접 — `light_infantry`와 스탯 동일 |
| 해골궁병 | `skel_archer` | `archer` | 23/21 | `skelarcher` | 암흑세력 원거리 — `light_archer`와 스탯 동일(오크 궁수 애셋이 없어 해골궁병으로 표현) |

`kind`가 알려지지 않은 값(오타)이면 로드 시 `push_error`로 경고한다(참조 무결성 — `_KNOWN_KINDS`).

**암흑세력 병종(`dark_hero`/`orc_infantry`/`skel_archer`)은 스탯이 인간 병종(`hero`/`light_infantry`/`light_archer`)과 동일**하다 — 순수 시각 변형이다(밸런스 영향 없음). 종족(theme) 필드·영웅별 개별 스프라이트는 미구현(향후).
...
| `sprite(arche) -> String` | 전투씬 스프라이트 세트 키 | 없으면 빈 문자열([LangBridge._cfg_side](../features/lang-battle.md) 소비) |
| `combat_stats(arche) -> Dictionary` | 전투 스탯 번들 | `{at, df, cmd_range, cmd_at, cmd_df, kind}` — [LangResolver](../features/lang-battle.md) 주입용(스프라이트 미포함) |
...
- [정상] `sprite` — `hero`=sword, `light_infantry`=soldier, `light_archer`=archer_a; `dark_hero`=eliteorc, `orc_infantry`=orc, `skel_archer`=skelarcher
- [정상] **암흑세력 병종은 인간 병종과 스탯 동일** — `combat_stats("orc_infantry")==combat_stats("light_infantry")`, `dark_hero==hero`, `skel_archer==light_archer`(kind·at/df·cmd 전부 일치, sprite만 다름); `skel_archer`는 원거리(`is_ranged`)
- [경계] 미지 아키타입 → kind/이동력/HP 0·빈 값, 원거리 아님, `sprite` 빈 문자열
