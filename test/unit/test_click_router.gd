...
##          → 캠프 메뉴 → 선택 해제.
## 인자 순서: resolve(on_party, on_camp, selected, reachable, info_open)

# --- 선택 중 이동 (건물 위 통행이 이 기능의 핵심) ---

func test_selected_reachable_empty_cell_moves() -> void:
	assert_eq(
		ClickRouter.resolve(false, false, true, true, false),
...
func test_party_cell_focuses_party() -> void:
	assert_eq(
		ClickRouter.resolve(true, false, false, false, false),
...
func test_party_cell_focuses_even_when_selected_and_reachable() -> void:
	...
	assert_eq(
		ClickRouter.resolve(true, false, true, true, false),
...
func test_party_on_camp_info_closed_focuses_party() -> void:
	...
	assert_eq(
		ClickRouter.resolve(true, true, false, false, false),
...
func test_party_on_camp_info_open_opens_menu() -> void:
	...
	assert_eq(
		ClickRouter.resolve(true, true, false, false, true),
...
func test_party_on_plain_info_open_still_focuses() -> void:
	...
	assert_eq(
		ClickRouter.resolve(true, false, false, false, true),
...
func test_camp_cell_opens_menu() -> void:
	assert_eq(
		ClickRouter.resolve(false, true, false, false, false),
		ClickRouter.CAMP_MENU, "캠프 칸(부대 아님·선택 아님) → 캠프 메뉴")

func test_empty_cell_deselects() -> void:
	assert_eq(
		ClickRouter.resolve(false, false, false, false, false),
		ClickRouter.DESELECT, "빈 칸(부대·캠프 아님) → 선택 해제")
