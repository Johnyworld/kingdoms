extends GutTest
## GameUnits — 게임 아키타입 카탈로그(단일 출처, res://data/units.csv).
## HP·시야·원거리·kind·전투 스탯(at/df·mv·cmd_range·cmd_at/cmd_df)이 모두 카탈로그 값임을 검증.

func test_archetype_specs() -> void:
	assert_eq(GameUnits.kind("hero"), "hero", "영웅 kind")
	assert_eq(GameUnits.kind("light_infantry"), "infantry", "경보병 kind")
	assert_eq(GameUnits.kind("light_archer"), "archer", "경궁병 kind")
...
func test_kind_mapping() -> void:
	assert_eq(GameUnits.kind("light_infantry"), "infantry", "근접 = infantry 병종")
	assert_eq(GameUnits.kind("light_archer"), "archer", "원거리 = archer 병종(근접 상성 페널티)")
	assert_eq(GameUnits.kind("hero"), "hero", "영웅 kind(상성 중립 — TypeAdvantage에 hero 행 없음)")

func test_combat_stats() -> void:
	# units.csv: 경보병·경궁병 = at23·df21·mv6·cmd_range3·cmd_at2·cmd_df2, 영웅 = at27·df24·mv6·cmd_range4·cmd_at2·cmd_df4.
	assert_eq(GameUnits.movement("light_infantry"), 6, "이동력")
	assert_eq(GameUnits.command_range("light_infantry"), 3, "지휘범위")
	assert_eq(GameUnits.movement("hero"), 6, "영웅 이동력")
	assert_eq(GameUnits.command_range("hero"), 4, "영웅 지휘범위")
	assert_eq(GameUnits.base_at("hero"), 27, "영웅 기본 AT")
	assert_eq(GameUnits.base_df("hero"), 24, "영웅 기본 DF")
	assert_eq(GameUnits.base_at("light_infantry"), 23, "경보병 기본 AT")

func test_combat_stats_bundle() -> void:
	# LangResolver 주입용 번들 — at/df/cmd_*/kind 를 한 번에.
	var s: Dictionary = GameUnits.combat_stats("hero")
...
func test_unknown_archetype() -> void:
	assert_eq(GameUnits.kind("없음"), "", "미지 아키타입 → 빈 kind")
